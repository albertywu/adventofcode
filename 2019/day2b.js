...
function getPoints(wire) {
  ...
  const position = [0, 0]
  points.add(serialize(position))
  for (let move of moves) {
    const [direction, count] = [move.slice(0, 1), Number(move.slice(1))]
    switch (direction) {
      case 'R':
        updatePosition(position, points, 0, count)
        break
      case 'L':
        updatePosition(position, points, 0, -1*count)
        break
      case 'U':
        updatePosition(position, points, 1, 1*count)
        break
      case 'D':
        updatePosition(position, points, 1, -1*count)
        break
...
function updatePosition(position, points, idx, count) {
  const sign = count > 0 ? 1 : -1
  for (let i = 0; i < Math.abs(count); i++) {
    // update
    position[0] = position[0] + (idx === 0 ? sign : 0)
    position[1] = position[1] + (idx === 1 ? sign : 0)
    // store
    points.add(serialize(position))
  }
}
