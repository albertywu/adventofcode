/*
Opcode:
BA01 a b c => adds a, b and puts result in address c
BA02 a b c => multiplies a, b and puts result in address c
3 a => takes integer as input, and saves it to position given by parameter
4 a => output value at address a (console.log)
99 => halt
*/
...
function process(program, input) {
  ...
  const output = []
  while (p[idx] !== 99) {
    console.log(idx, p.slice(0, idx), p.slice(idx, idx + 4))
    if (p[idx] === 3) { // handle input
      ...
      p[p[idx + 3]] = p[p[idx + 1]] * p[p[idx + 2]]
      idx += 4
    } else if (String(p[idx]).endsWith('01')) {
      // handle addition with modes
      const instruction = String(p[idx]).padStart(4, '0')
      ...
      idx += 4
    } else if (String(p[idx]).endsWith('02')) {
      ...
      idx += 2
    } else if (String(p[idx]).endsWith('04')) {
      const instruction = String(p[idx]).padStart(3, '0')
      const [mode, _] = [...instruction]
      output.push(p[
        mode === '0' ? p[idx + 1] : idx + 1
      ])
      idx += 2
    }
  }
