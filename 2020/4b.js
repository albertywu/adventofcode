...
for (const passport of passports) {
  if (isValid(passport)) numValidPassports++;
}
console.log(numValidPassports);

function isValidByr(x) {
  const isFourDigits = x.length === 4;
  const isInRange = Number(x) >= 1920 && Number(x) <= 2002;
  return isFourDigits && isInRange;
}

function isValidIyr(x) {
  const isFourDigits = x.length === 4;
  const isInRange = Number(x) >= 2010 && Number(x) <= 2020;
  return isFourDigits && isInRange;
}

function isValidEyr(x) {
  const isFourDigits = x.length === 4;
  const isInRange = Number(x) >= 2020 && Number(x) <= 2030;
  return isFourDigits && isInRange;
}

function isValidHgt(x) {
  const suffix = x.slice(x.length - 2);
  if (suffix !== "cm" && suffix !== "in") return false;
  const height = Number(x.slice(0, x.length - 2));
  return (
...
function isValidHcl(x) {
  return /^#[0-9a-f]{6}$/.test(x);
}

function isValidEcl(x) {
  return /^(amb|blu|brn|gry|grn|hzl|oth)$/.test(x);
}

function isValidPid(x) {
  return /^[0-9]{9}$/.test(x);
}
...
ecl:oth hcl:#602927 eyr:2025 iyr:2013 hgt:151cm byr:1992 pid:812583062`
    .split("\n\n")
    .map((passport) => {
      return passport.split(/\s+/).reduce((prev, curr) => {
