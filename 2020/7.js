...
for (const bag of Object.keys(bags)) {
  if (containsShinyGold(bags, bag)) count++;
}
console.log(count);

function containsShinyGold(bags, target) {
  if (Object.keys(bags[target]).length === 0) return false;
  const hasShinyGold = "shiny-gold" in bags[target];
  const children = Object.keys(bags[target]);
  return hasShinyGold || children.some((c) => containsShinyGold(bags, c));
}

function getBags() {
  const input = `shiny indigo bags contain 4 vibrant lime bags.
...
light teal bags contain 3 faded green bags, 5 dark indigo bags, 1 pale gold bag.`;
  ...
  for (const line of input.split("\n")) {
    const [_, adj1, adj2, __, rest] = line.match(
      /^(.+) (.+) (.+) contain (.+)\.$/
    );
    if (rest === "no other bags") {
      result[`${adj1}-${adj2}`] = {};
    } else {
      result[`${adj1}-${adj2}`] = parseBags(rest);
    }
  }
  return result;
}

// 1 dim beige bag, 2 shiny aqua bags, 1 faded gray bag
function parseBags(s) {
  const parts = s.split(", ").map((x) => x.trim());
  let result = {};
  for (const part of parts) {
    const [_, qty, adj1, adj2] = part.match(/^([0-9]+) ([a-z]+) ([a-z]+) bag/);
    const key = `${adj1}-${adj2}`;
    result[key] = Number(qty);
  }
  return result;
}
