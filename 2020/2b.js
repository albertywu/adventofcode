let numValid = 0;
for (const [min, max, letter, password] of parseInput()) {
...
function isValid(min, max, target, password) {
  let count = 0;
  const letters = password.split("");
  const a = letters[min - 1];
  const b = letters[max - 1];
  return xor(a === target, b === target);
}
...
9-13 n: nglxnhnnrktnnfznn
  `
    ...
    .map((line) => {
      ...
      const [min, max] = minMax.split("-").map((_) => Number(_));
      const letter = letterColon.slice(0, 1);
