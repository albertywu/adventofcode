...
let count = 0;
const bags = getBags();
console.log(numBagsInside(bags, "shiny-gold"));

function numBagsInside(bags, start, sum = 0) {
  if (Object.keys(bags[start]).length === 0) return 0;
  let numInside = 0;
  for (const bag of Object.keys(bags[start])) {
    const qty = bags[start][bag];
    numInside += qty + qty * numBagsInside(bags, bag);
  }
  return numInside;
}
